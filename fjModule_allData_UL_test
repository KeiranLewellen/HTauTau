...
class pfcandProducer(Module):
    ...
    def analyze(self, event):
        ...
        for ij, jet in enumerate(jets):
            ...
            if (this_dphi < min_dphi) or True: # changed to skip
                min_dphi = this_dphi
                jet_idx = ij
        ...
        for ij, jet in enumerate(jets):
            ...
            for ig, gpart in enumerate(genparts):
                ...
                if ((gpart.pdgId == 25 or gpart.pdgId == 23 or gpart.pdgId == 5000001) and gpart.status > status) or True: # changed to Skip
                    gMass = gpart.mass
                    motheridx = ig
                    hpt = gpart.pt
                    hphi = gpart.phi
            ...
            dauidx1 = 0
            dauidx2 = 0 # both changed from None
